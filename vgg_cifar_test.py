...
def train_vgg_cifar10(use_simplified=True, epochs=10, batch_size=32, learning_rate=0.001, val_size=5000):
    """
    Train VGG-16 on CIFAR-10

    Args:
        use_simplified: If True, use simplified VGG with fewer filters
    ...
    # Build network
    print(f"\nBuilding VGG-16 network ({'simplified' if use_simplified else 'full'})...")
    if use_simplified:
        network = build_vgg16_simplified_cifar10()
    else:
        network = build_vgg16_cifar10()
    ...
    for epoch in range(epochs):
        ...
        for batch_idx in range(n_batches):
            ...
            if (batch_idx + 1) % 50 == 0:
                ...
                print(f"  Epoch {epoch+1}/{epochs} - Batch {batch_idx+1}/{n_batches} - Loss: {avg_batch_loss:.4f}")
...
if __name__ == "__main__":
    # Train simplified VGG-16 (recommended for faster training)
    train_vgg_cifar10(
        use_simplified=True,
        epochs=10,
        batch_size=32,
